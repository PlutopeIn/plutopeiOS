...
class ImportWalletViewController: UIViewController {
    ...
    @IBOutlet weak var lblSelectPhrase: UILabel!
    ...
    fileprivate func uiSetUp() {
        ...
        self.lblWalletName.text = LocalizationSystem.sharedInstance.localizedStringForKey(key: LocalizationLanguageStrings.waletName, comment: "")
        self.lblSelectPhrase.text = LocalizationSystem.sharedInstance.localizedStringForKey(key: LocalizationLanguageStrings.selectPhase, comment: "")
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        txtWalletName.text = "Main Wallet \(walletCount)"
...
//        txtViewPhrase.placeholderLabel.textColor = UIColor.placeholderText
        txtViewPhrase.placeholderLabel.textColor = UIColor.label
    ...
    fileprivate func activeTokenList() {
        ...
        if let tokensList = self.tokensList, let activeTokens = self.activeToken {
            print("activeToken",self.activeToken)
    ...
    fileprivate func walletCreation(_ coinList: [CoingechoCoinList]?,_ modifiedPhrase: String,alreadyAddStaticToken:Bool) {
        WalletData.shared.createWallet(walletName: self.txtWalletName.text ?? "", mnemonicKey: modifiedPhrase, isPrimary: true, isICloudBackup: false, isManualBackup: true,coinList: coinList,alreadyAddStaticToken: alreadyAddStaticToken) { success in
            if success {
                ...
                print("self.primaryWallet",self.primaryWallet)
...
//                    }
                    ...
                    self.registerUserViewModel.registerAPI(walletAddress: WalletData.shared.myWallet?.address ?? "", appType: 1,deviceId: deviceId,fcmToken: fcmtoken ?? "", type: "import", referralCode: "") { resStatus, message in
                       // if resStatus == 1 {
                          //  self.showToast(message: message, font: .systemFont(ofSize: 15))
            ...
            if DatabaseHelper.shared.entityIsEmpty("Token") {
                viewModel.apiCoinList { status, _, coinList in
                    ...
                    if status {
                        ...
                        self.walletCreation(coinListD,modifiedPhrase ?? "", alreadyAddStaticToken: false)
                    } else {
                        self.btnImport.HideLoader()
                    }
                }
            } else {
                self.walletCreation(nil, modifiedPhrase ?? "", alreadyAddStaticToken: true)
            }
        }
    }
