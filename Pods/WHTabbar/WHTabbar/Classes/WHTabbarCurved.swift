...
extension WHTabbarController {
    ...
    public func makeTabbarCurved() {
        ...
        // Adjust the tab bar frame
        let tabBarWidth = self.view.bounds.width - 40
        let tabBarHeight = self.tabBar.frame.height
        let tabBarX = (self.view.bounds.width - tabBarWidth) / 2
        let tabBarY = self.view.bounds.height - tabBarHeight
        self.tabBar.frame = CGRect(x: tabBarX, y: tabBarY, width: tabBarWidth, height: tabBarHeight)
