...
//            let formattedString = WalletData.shared.formatDecimalString("\(data.price ?? "")", decimalPlaces: 8)
...
//            let priceChangeImpactTruncatedValue = WalletData.shared.formatDecimalString("\(priceChangeImpact)", decimalPlaces: 2)
...
//    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let data = chainTokenList?[indexPath.row] {
            ...
            if LocalizationSystem.sharedInstance.getLanguage() == "ar" {
                cell.lblCoinName.textAlignment = .right
                cell.lblSymbol.textAlignment = .right
               
            } else {
                cell.lblCoinName.textAlignment = .left
                cell.lblSymbol.textAlignment = .left
            }
            cell.lblYourTokens.text = LocalizationSystem.sharedInstance.localizedStringForKey(key: LocalizationLanguageStrings.yourtokens, comment: "")
            cell.lblCoinName.text = data.symbol
            cell.lblSymbol.text = data.type
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        let outerView = UIView(frame: CGRect(x: 0, y: 0, width: screenWidth - 32, height: 110))
