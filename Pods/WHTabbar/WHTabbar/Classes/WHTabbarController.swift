...
open class WHTabbarController: UITabBarController {
    ...
    open override func viewDidLoad() {
        ...
        if #available(iOS 11.0, *) {
               ...
               let tabBarFrame = CGRect(x: safeAreaInsets.left + 20, y: view.bounds.height - tabBar.frame.height - safeAreaInsets.bottom, width: view.bounds.width - (safeAreaInsets.left + safeAreaInsets.right + 40), height: tabBar.frame.height)
               self.tabBar.frame = tabBarFrame
               self.tabBar.layer.cornerRadius = tabBarFrame.height / 2
           } else {
               let tabBarFrame = CGRect(x: 20, y: view.bounds.height - tabBar.frame.height, width: view.bounds.width - 40, height: tabBar.frame.height)
               self.tabBar.frame = tabBarFrame
               self.tabBar.layer.cornerRadius = tabBarFrame.height / 2
           }
    }
