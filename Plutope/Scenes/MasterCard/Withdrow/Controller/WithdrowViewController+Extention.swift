//
//  WithdrowViewController+Extention.swift
//  Plutope
//
...
extension WithdrowViewController : UITextFieldDelegate {
    ...
    @objc private func apiCall() {
        ...
        _ =  WalletData.shared.formatDecimalString("\(balanceDouble ?? 0.0)", decimalPlaces: 6)
       ...
       if Double(txtPay.text ?? "") ?? 0.0 > Double(self.lblMax.text ?? "") ?? 0.0 {
        ...
        } else if Double(txtPay.text ?? "") ?? 0.0 > balanceDouble ?? 0.0 {
            self.btnContinue.alpha = 0.5
            self.btnContinue.isEnabled = false
            lblPayError.isHidden = false
            txtPay.textColor = UIColor.red
            lblPayError.text = LocalizationSystem.sharedInstance.localizedStringForKey(key: LocalizationLanguageStrings.lowBalanceError, comment: "")
            lblGetError.isHidden = false
            txtGet.textColor = UIColor.red
            lblGetError.text = LocalizationSystem.sharedInstance.localizedStringForKey(key: LocalizationLanguageStrings.lowBalanceError, comment: "")
    ...
    /// getBestPrice
     func getBestPrice() {
         DispatchQueue.main.async {
             ...
             var tokenCurrency = ""
             var tokenName = ""
             if let pair = self.arrWalletPairs.first(where: { $0.currencyFrom == selectedToken }) {
...
extension WithdrowViewController {
